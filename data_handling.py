...
def load_data(nesda_file=NESDA_FILE, nesda_file_dtype=NESDA_FILE_DTYPE, target_col='persistance_anxiety',
              ...
              print_diagnostic=True, use_summary=False):
    ...
    if use_summary:
        ...
        var_names = np.array(df.columns, dtype=str)
        summary_col = var_names[np.char.find(var_names, 'summary') != -1]
    n_summary_dropped = len(summary_col)
    print('Summary variables to drop: {}'.format(n_summary_dropped))

    df.drop(list(col_to_remove) + list(summary_col), axis='columns', inplace=True)
    df_dtypes = remove_var_names(df_dtypes, list(col_to_remove) + list(summary_col))
    ...
    return df, df_dtypes, y, n_summary_dropped
...
def drop_subj_with_many_missing(df, y, target_var, n_summary_cols, missing_thresh=0.2, missing_val=-9):
    missing_proportion = (df == missing_val).sum(axis=1)/(df.shape[1] + n_summary_cols)
...
def missing_data_handling(target_col='persistance_anxiety', load_df='', load_df_dtypes='', load_df_labels='',
    ...
    if load_df and load_df_dtypes and load_df_labels:
        ...
        df, df_dtypes, y, n_summary_col = load_data(target_col=target_col, use_summary=use_summary)
        ...
        df, y = drop_subj_with_many_missing(df, y, n_summary_cols=n_summary_col, target_var=target_col,
                                            missing_thresh=0.2)
...
if __name__ == '__main__':
    _, _, _ = get_data(modality_name='all', use_summary=False, make_plots=True)
