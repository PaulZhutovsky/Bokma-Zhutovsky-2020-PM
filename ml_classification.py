...
def create_labels(y, type_of_analysis):
    ...
    if type_of_analysis == 'any_disorder':
        ...
        y['clf_labels'] = y.persistance_anxiety.values
    elif type_of_analysis == 'reviewer2_question3':
        # special case: you train on the "any_disorder" labels but predict on the "any_anxiety" labels, i.e. you need
        # two y vectors
        y_anxiety = y.persistance_anxiety.values

        id_other = (y.pureanxiety == 'comorbid->pure_other') | \
                   (y.pureanxiety == 'pure_anxiety->pure_other')
        y.persistance_anxiety.loc[id_other] = 1
        y_other = y.persistance_anxiety.values
        import ipdb; ipdb.set_trace()
        y = np.column_stack((y_other, y_anxiety))
        return y, multiclass
    else:
...
def run_cross_validation(df_X, y, cat_vars, other_vars, n_jobs, random_state, n_jobs_rf=1, verbose=1,
    ...
    y_tmp = y
    if y.ndim == 2:
        # reviewer 2 question 3: train on "any_disorder" labels and test on "any_anxiety" labels
        # doing here the most "optimal" thing (maybe) stratifying across all 4 label combinations
        y_tmp = np.char.add(y_tmp[:, 0].astype(str), y_tmp[:, 1].astype(str))
    res = Parallel(n_jobs=n_jobs,
                   ...
                   max_nbytes='50M')(delayed(one_cv_run_ref)(clone(estimator), df_X, y, train, test, i_cv + 1, cat_vars,
                                     ...
                                     for i_cv, (train, test) in enumerate(cv.split(df_X, y_tmp)))
    return res
...
def one_cv_run_ref(estimator, X, y, id_train, id_test, i_cv, cat_vars, other_vars, cat_encoding=None, multiclass=False):
    if y.ndim == 2:
        # reviewer 2 question 3: train on "any_anxiety" and test on "any_disorder"
        y_train, y_test = y[id_train, 0], y[id_test, 1]
    else:
